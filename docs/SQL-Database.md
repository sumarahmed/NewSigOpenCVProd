...
The API also supports the `Verifier` role through `SIGNATURE_API_KEYS`. The API fails startup if no key configuration is supplied through `SIGNATURE_API_KEYS` or `SignatureVerification:ApiKeys`.
...
- Back up the `SignatureVerification` database with the customer-approved SQL backup process.
